...
const UserProfile10 = ({ doctor, whatsapp, className }: UserProfile10Props) => {
  return (
    <Card className={cn("w-full gap-0 overflow-hidden py-0 shadow-lg shadow-navy/5", className)}>
      ...
      <div className="relative aspect-[4/5] w-full overflow-hidden bg-accent">
        <img
          src={doctor.imageSrc}
          alt={`${doctor.name} — ${doctor.role} en JS Dental Group`}
          loading="lazy"
          width={480}
          height={600}
      ...
      <CardContent className="space-y-4 p-5">
        <div>
          <div className="flex items-center gap-1.5">
            ...
            {doctor.verified && (
              <BadgeCheck className="size-5 shrink-0 fill-teal text-card" aria-label="Especialista certificado" />
            )}
          </div>
        ...
        <div className="flex items-center justify-between border-t pt-4">
          <span className="text-xs font-semibold tracking-widest text-teal uppercase">
            {doctor.verified ? "Especialista" : "Equipo"}
          </span>
...
const TeamProfiles = ({ doctors, whatsapp, className }: TeamProfilesProps) => {
  return (
      ...
      <div className="container">
        <div className="mx-auto mb-14 flex max-w-xl flex-col gap-4 text-center" data-animate="up">
          ...
          <p className="text-xl text-muted-foreground">
            Un equipo multidisciplinario que trabaja cada caso en conjunto.
          </p>
        </div>
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4" data-stagger>
          {doctors.map((doctor) => (
            <UserProfile10 key={doctor.name} doctor={doctor} whatsapp={whatsapp} />
          ))}
        </div>
      </div>
    </section>
